...
# 這裡是多機台設定：大轉輪x2, 坐推x3, 漫步機x1
if "equipment_status" not in st.session_state: 
    equip_config = {"大轉輪": 2, "坐推": 3, "漫步機": 1}
    st.session_state.equipment_status = {f"{name}_{i+1}": None for name, count in equip_config.items() for i in range(count)}
...
with st.sidebar:
    ...
    if st.button("🚀 注入 20 位長輩數據"):
        for i in range(20):
            p_id = get_or_create_patient_id("測試", str(i), 70)
            add_patient(p_id, "測試", str(i), 70, ["大轉輪", "坐推"])
        st.rerun()
...
with left_col:
    ...
    if not st.session_state.waiting_queue:
        ...
        st.dataframe(df[["id", "name", "target_equip"]].rename(columns={"id":"ID", "name":"姓名", "target_equip":"器材"}), use_container_width=True)
    ...
    for eq, p in st.session_state.equipment_status.items():
        with st.container():
            if p:
                ...
                if st.button(f"結束使用 {eq}", key=f"f_{eq}"):
                    st.session_state.equipment_status[eq] = None
                    st.rerun()
            else:
                st.markdown(f"<div class='status-card' style='border-left: 5px solid #cbd5e1;'><b>⚙️ {eq}</b><br>🟢 空閒中</div>", unsafe_allow_html=True)

if any([len(st.session_state.waiting_queue) > 0, any(st.session_state.equipment_status.values())]):
    time.sleep(1)
    st.rerun()